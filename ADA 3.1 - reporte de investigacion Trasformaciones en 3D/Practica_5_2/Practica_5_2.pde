...
void draw() {
  ...
  fill(255, 204, 0);
  noStroke();
  ellipse(width / 2, height / 2, 50, 50); // Sol en el centro

  for (int i = 0; i < numPlanetas; i++) {
    // Dibujar órbita
    noFill();
    stroke(255, 100);
    ellipse(width / 2, height / 2, distancias[i] * 2, distancias[i] * 2);

    // Calcular posición del planeta
    float x = width / 2 + cos(angulos[i]) * distancias[i];
    float y = height / 2 + sin(angulos[i]) * distancias[i];
